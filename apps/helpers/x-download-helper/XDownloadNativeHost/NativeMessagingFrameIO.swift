...
enum NativeMessagingFrameError: Error {
    case endOfStream
    case truncated
    case oversized
}
...
enum NativeMessagingFrameReader {
    ...
    private static func readExactly(_ count: Int, from handle: FileHandle) throws -> Data {
        ...
        while result.count < count {
            ...
            guard !part.isEmpty else { throw result.isEmpty ? NativeMessagingFrameError.endOfStream : NativeMessagingFrameError.truncated }
            result.append(part)
        }
        return result
    }
...
enum NativeMessagingFrameWriter {
    static func writeMessage(_ payload: Data, to handle: FileHandle) throws {
        ...
        var frame = Data([UInt8(length & 0xff), UInt8((length >> 8) & 0xff), UInt8((length >> 16) & 0xff), UInt8((length >> 24) & 0xff)])
